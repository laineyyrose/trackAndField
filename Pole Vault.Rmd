...
```{r}
womensSimplified <- newWomens %>% select(DATE, BIRTHDATE, HEIGHT, NAME) %>% drop_na(DATE, BIRTHDATE, HEIGHT, NAME)

mensSimplified <- newMens %>% select(DATE, BIRTHDATE, HEIGHT, NAME) %>% drop_na(DATE, BIRTHDATE, HEIGHT, NAME) 
```
...
```{r}
womenRevised %>% mutate(AGE = YEAR - BIRTHYEAR - 1900)

menRevised %>% mutate(AGE = YEAR - BIRTHYEAR - 1900)
```

I now will need to remove trailing letters on the height so that I can correctly plot it.

```{r}
womenRevised$HEIGHT <- gsub('A','', womenRevised$HEIGHT)

menRevised$HEIGHT <- gsub('A', '', menRevised$HEIGHT)
```
...
```{r}
library(ggplot2)
ggplot(womenRevised, aes(x = AGE, y = HEIGHT)) + geom_point()

ggplot(menRevised, aes(x = AGE, y = HEIGHT)) + geom_point()
